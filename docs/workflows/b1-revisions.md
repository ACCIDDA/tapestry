...
them; the prelaunch audit found no exactly zero errors on combined natural recent
support. It did not test near-zero numerical denominators; the results review found
two such groups, so this check was insufficient for stability.
...
- 2026-09-18: all 160 runs complete (L40 1540593, H100 1540594, ~6:02 each).
  Results are on `docs/results/b1-overnight/index.md`. The subsequent review corrected
  the report generator and page: target gated improves matched mixed-mask B but
  not the best gap-only control; the same claim does not hold on pathogen MLP.
  Gated is the strongest recent-head family, but whether two-stage beats the report
  depends on aggregation. Two near-zero positive baseline groups destabilize the
  registered location-relative nowcast metric. Excluding them at native total WIS
  <1e-6 is a post-hoc sensitivity, not a silent scorer fix. Exact configuration
  ordering changes under pooling, and pooling changes the geographic estimand.
  Native-unit counts/proportions must not be pooled into reconstruction skill;
  new matched Q95-normalized diagnostics retain all report groups and both ages.
  Similar outage scores are not a scorer artifact: recent heads execute, and
  near-zero admission predictions require architectural investigation. Error bars
  on paired contrasts are descriptive SD, not confidence intervals.
