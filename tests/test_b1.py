...
def test_matched_final_control_changes_only_recent_input_values():
    """The input-cost control must differ from B1 in recent values and nothing else.

    If availability, labels or episode support moved too, a paired run against it
    would measure those as well, which is exactly the confound the control exists
    to remove.
    """
    import numpy as np
    import sys
    ...
    from build_b1_matched_final import matched_final_arrays

    ds = _synthetic_fold_dataset()
    arrays = {k: v.copy() for k, v in ds.arrays.items()}
    # Make the last two context weeks look like partially-reported Wednesday versions.
    arrays['X_final'][:, -2:] = False
    arrays['X_available'][:, -2:] = True
    arrays['X_values'][:, -2:] *= 0.5
    controlled, audit = matched_final_arrays(arrays)

    assert audit['replaced_cells'] > 0
    # The fixture's calendar edges have recent cells with no pinned final. Those keep
    # their version rather than being invented, and are counted, not dropped.
    assert audit['replaced_cells'] + audit['unresolved_vintage_cells'] == audit['vintage_cells']
    unresolved = (arrays['X_available'][:, -2:] & ~arrays['X_final'][:, -2:]
                  & ~arrays['Y_recent_valid'])
    np.testing.assert_allclose(controlled['X_values'][:, -2:][unresolved],
                               arrays['X_values'][:, -2:][unresolved])
    assert not controlled['X_final'][:, -2:][unresolved].any(), 'a kept version must not be flagged final'
    # Availability, labels and older context are untouched.
    for key in ('X_available', 'Y_recent', 'Y_recent_valid', 'Y_future', 'Y_future_valid',
                'context_dates', 'target_dates', 'issuance_dates'):
        assert np.array_equal(arrays[key], controlled[key]), f'{key} must not change'
    assert np.array_equal(arrays['X_values'][:, :-2], controlled['X_values'][:, :-2])
    # Replaced recent cells now equal the pinned final and are flagged as final.
    replaced = arrays['X_available'][:, -2:] & ~arrays['X_final'][:, -2:] & arrays['Y_recent_valid']
    np.testing.assert_allclose(controlled['X_values'][:, -2:][replaced],
                               arrays['Y_recent'][replaced])
    assert controlled['X_final'][:, -2:][replaced].all()
